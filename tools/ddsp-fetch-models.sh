...
# third-party release). This script reproduces them:
#
#   1. download DDSP-VST-Models.zip  (Apache-2.0, storage.googleapis.com/ddsp-vst)
#   2. extract Trumpet.tflite + Clarinet.tflite
#   3. run tools/ddsp-convert.py (in an ephemeral uv env) -> <pack>/backend/*.{ddspw,ref}
#
# Idempotent: exits early if all four outputs already exist. FORCE=1 re-converts.
#
...
PACK="instrument-packs/hello-ddsp"
DEST="$PACK/backend"
...
# model name in the zip  ->  output basename in the pack
MODELS=( "Trumpet:trumpet" "Clarinet:clarinet" )

have_all=1
for m in "${MODELS[@]}"; do
  base="${m#*:}"
  [[ -s "$DEST/$base.ddspw" && -s "$DEST/$base.ref" ]] || have_all=0
done
if [[ "$have_all" == 1 && "${FORCE:-0}" != "1" ]]; then
  echo "==> DDSP models present (FORCE=1 to re-convert): $DEST/*.ddspw"
  exit 0
fi
...
echo "==> converting (uv run, ephemeral Python 3.11 env)"
for m in "${MODELS[@]}"; do
  name="${m%%:*}"; base="${m#*:}"
  uv run --quiet --python 3.11 \
    --with 'numpy<2' --with tflite --with ai-edge-litert \
    python tools/ddsp-convert.py "$TFDIR/$name.tflite" "$DEST/$base"
done

echo "==> done:"
ls -la "$DEST"/*.ddspw "$DEST"/*.ref
